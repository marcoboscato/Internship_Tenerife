...
def completeness(yy, plot=True):
	"""Account for the completeness using GUMS catalogs
...
if __name__ == "__main__":
	...
	# read master spectroscopic table
	st = Table.read(path+'utils/spec/spec_stars_for_EDR3_v3_gb_noduplicates.txt',format='ascii')
	inter,ia,ib=np.intersect1d(mt['name'].data,st['id'].data,return_indices=True)
	mt = mt[ia]

	listGlx=mt['name'].data
	
	for index in np.arange(0,len(listGlx),1):
		...
		# load galaxy table with spectroscopic values
		if not os.path.exists(path+'catalogs/spec/'+glx+'_gaiaeDR3_spec_test.fits'):
			# xmatch using STILTS
			os.system("/home/floyd/stilts/stilts tskymatch2 ifmt1=fits ifmt2=fits omode=out out={0} ofmt=fits ra1=ra dec1=dec ra2=RAJ2000 dec2=DEJ2000 error=1.0 join=all1 find=best in1={1} in2={2}".format(path+'catalogs/spec/'+glx+'_gaiaeDR3_spec_test.fits', path+'catalogs/'+glx+'_gaiaeDR3.fits', path+'catalogs/spec/sel_'+glx+'_spec.fits'))
		
		t = load_GaiaEDR3_table('catalogs/spec/'+glx+'_gaiaeDR3_spec_test.fits', path=path, glxParallax=glxParallax, glxParallax_e=glxParallax_e)
		
		
		# Radial velocities
		
		# mask invalid entries
		vlos  = np.ma.masked_invalid(t['rvh'].data).filled(0.)
		evlos = np.ma.masked_invalid(t['ervh'].data).filled(9999.)
		
		# set outliers to 0 vlos and high error to reduce their weight into the Likelihood
		vlos[np.where((abs(vlos)>=800.)|(abs(vlos)<=0.001))] = 0.
		evlos[np.where((abs(vlos)>=800.)|(abs(vlos)<=0.001))] = 9999.
		
		# set average velocity values for target galaxy
		vmean = mt['av_vlos'][index]
		if mt['sig_v_vlos'][index]>0.:
			disp = mt['sig_v_vlos'][index]
		else:
			disp = 5. #mt['e_sig_v_vlos_plus'][index] #dispersion upper limit
		...
		##################
		# Vlos probability
		Plos  = (1./np.sqrt(2*np.pi*(evlos[ix]**2 + disp**2)))*np.exp(-0.5*(vlos[ix]-vmean)**2/(evlos[ix]**2 + disp**2)) 
		...
		#-------------------------------------	
		# number of dimensions our problem has
		parameters = ["pmra","pmdec","frac","meanV_MW","dispV_MW"]
		labels     = [r"${PM}_{\rm RA}$",r"${PM}_{\rm DEC}$","q",r"$V_{\rm MW}$",r"$\sigma_{\rm v, MW}$"]
		# run the MultiNest
		...
		header=('PM_RA\tPM_DEC\tQ\tV_MW\tD_MW\tePM_RA_d\tePM_DEC_d\teQ_d\teV_MW_d\teD_MWd\tePM_RA_u\tePM_DEC_u\teQ_u\teV_MW_u\teD_MWu\tlnZ\tC'))
		
		# set output probabilities
		Like_PM = LikePM(outMod[0],outMod[1])
		cPlos = (1./np.sqrt(2*np.pi*(evrad**2 + outMod[4]**2)))*np.exp(-0.5*(vrad-outMod[3])**2/(evrad**2 + outMod[4]**2))
		
		P_i = outMod[2]*(Like_PM*Pcmd*Prad*Plos)/(outMod[2]*(Like_PM*Pcmd*Prad*Plos) + (1.-outMod[2])*(cPrad*cPcmd*cPpmt*cPlos))
		...
		fig, ((ax1, ax2), (ax3, ax4)) = plt.subplots(2, 2, constrained_layout=True, figsize=(13, 11))
		fig.suptitle(glx,fontsize=fsize+6)
		#P1
		...
		#P4
		ax4.scatter(tRad[ix][ipd][vlos[ix][ipd]!=0.],vlos[ix][ipd][vlos[ix][ipd]!=0.],s=10,c='gray')
		ax4.scatter(tRad[ix][ipu][vlos[ix][ipu]!=0.],vlos[ix][ipu][vlos[ix][ipu]!=0.],c=P_i[ipu][vlos[ix][ipu]!=0.],s=25,cmap='jet')
		ax4.set_xlabel(tRad.description,fontsize=fsize+2)
		ax4.set_ylabel(r'V$_{\rm los}$ [km s$^{-1}$]',fontsize=fsize+2)
		...
		if mt['type'][index]=='UFD':
			...
			normed_H=synth_hist2D(xx=xx,yy=yy,DM=glxDM,DM_e=glxDM_e, G=G, sigBP=sigBP, sigRP=sigRP, plot=False, pathSynth=path+'utils/synth_ufd_eGDR3.fits')
